# coding: utf-8

"""
    Gusto API

    Welcome to Gusto's Embedded Payroll API documentation!

    The version of the OpenAPI document: 2024-03-01
    Contact: developer@gusto.com
    Generated by: https://konfigthis.com
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING
from pydantic import BaseModel, Field, RootModel, ConfigDict

from gusto_app_integrations_python_sdk.pydantic.signatory_home_address import SignatoryHomeAddress

class Signatory(BaseModel):
    title: typing.Optional[typing.Optional[str]] = Field(None, alias='title')

    # The current version of the signatory. See the [versioning guide](https://docs.gusto.com/embedded-payroll/docs/idempotency) for information on how to use this field.
    version: typing.Optional[str] = Field(None, alias='version')

    uuid: typing.Optional[str] = Field(None, alias='uuid')

    first_name: typing.Optional[typing.Optional[str]] = Field(None, alias='first_name')

    last_name: typing.Optional[typing.Optional[str]] = Field(None, alias='last_name')

    phone: typing.Optional[typing.Optional[str]] = Field(None, alias='phone')

    email: typing.Optional[str] = Field(None, alias='email')

    birthday: typing.Optional[typing.Optional[str]] = Field(None, alias='birthday')

    # Whether or not the signatory is also the payroll admin of the company.
    is_admin: typing.Optional[bool] = Field(None, alias='is_admin')

    # Indicates whether the signatory has an SSN in Gusto.
    has_ssn: typing.Optional[bool] = Field(None, alias='has_ssn')

    # |   |   | |---|---| |__Status__| __Description__ | | Pass | Signatory can sign all forms | | Fail | Signatory cannot sign forms | | Skipped | Signatory cannot sign Form 8655 until the form is manually uploaded as wet-signed | | null | Identity verification process has not been completed |
    identity_verification_status: typing.Optional[Literal["Pass", "Fail", "Skipped"]] = Field(None, alias='identity_verification_status')

    home_address: typing.Optional[SignatoryHomeAddress] = Field(None, alias='home_address')

    model_config = ConfigDict(
        protected_namespaces=(),
        arbitrary_types_allowed=True
    )
